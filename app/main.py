...
def item_does_exist_handler(request: Request, exc: ItemDoesExistError) -> JSONResponse:
    return JSONResponse(
...
def field_is_empty_handler(request: Request, exc: FieldIsEmptyError) -> JSONResponse:
...
def item_not_owned_handler(request: Request, exc: ItemNotOwnedError) -> JSONResponse:
...
def not_enough_money_handler(
    ...
    return JSONResponse(status_code=409, content={"error": "User has not enough money"})


@app.exception_handler(UserDoesExistError)
def user_does_exist_handler(request: Request, exc: UserDoesExistError) -> JSONResponse:
